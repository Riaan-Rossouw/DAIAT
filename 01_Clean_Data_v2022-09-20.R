...
pop <- pop %>% select(1:5) %>% left_join(CountryCodes %>% select(3:4), by = c("Country_iso" = "alpha_3_code"))
...
hemisphere <- hemisphere %>% select(1:3) %>% left_join(CountryCodes %>% select(2,4), by = c("country_code"="alpha_2_code"))
...
FAO <- FAO %>% select(1:10) %>% left_join(FAOtoHS %>% select(1,3), by = c("Item Code"="FAO_Code"))
FAO <- as.data.table(subset(FAO, select = c(10,11,7,6,9)))
...
AGST.data[,Country_iso := NULL]
AGST.data <- AGST.data %>% select(1:7) %>% left_join(geo %>% select(1,3,4,7), by = c("Country"="Country_num"))
AGST.data <- AGST.data %>% select(1:10) %>% left_join(pop %>% select(1:4), by = c("Country"="Country_num","Year"="Year"))
AGST.data <- AGST.data %>% select(1:12) %>% left_join(ck %>% select(1:3), by = c("Country"="Country_num","Year"="Year"))
AGST.data <- AGST.data %>% select(1:13) %>% left_join(Real_TO %>% select(1,4:5), by = c("Country"="iso_num","Year"="Year"))
AGST.data <- AGST.data %>% select(1:14) %>% left_join(hemisphere %>% select(1:3), by = c("Country"="Country_num"))
AGST.data <- AGST.data %>% select(1:16) %>% left_join(wto_com %>% select(1,3:4), by = c("Country"="Members","Year"="Year"))
AGST.data <- AGST.data %>% select(1:17) %>% left_join(TFP, by = c("Country"="Country_num","Year"="Year"))
AGST.data <- AGST.data %>% select(1:18) %>% left_join(FAO %>% select(1:11), by = c("Country"="country_num","HS6"="hs6","Year"="year"))
colnames(AGST.data)[19] <- "FAO_Area_Harvested"
colnames(AGST.data)[20] <- "FAO_Laying"
colnames(AGST.data)[21] <- "FAO_Milk_Animals"
colnames(AGST.data)[22] <- "FAO_Producing_Animals_Slaughtered"
colnames(AGST.data)[23] <- "FAO_Production"
colnames(AGST.data)[24] <- "FAO_Stocks"
colnames(AGST.data)[25] <- "FAO_Yield"
colnames(AGST.data)[26] <- "FAO_Yield_Carcass_Weight"
AGST.data <- AGST.data %>% select(1:26) %>% left_join(AQUA, by = c("Country"="Country_num","Year"="Year"))
AGST.data <- AGST.data %>% select(1:31) %>% left_join(ICC %>% select(1:2,4:7), by = c("Country"="Country_num","HS6"="HS6","Month"="Month"))
...
#AGST.data <- AGST.data %>% select(1:34) %>% left_join(dist.new %>% select(1,3:9), by = c("Country"="iso_o_num","Year"="year"))
