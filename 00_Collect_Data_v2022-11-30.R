...
# Bind headers and downloaded data
x <- as.data.table(do.call(rbind, dx_temp))
comtrade.data <- setDT(x)[trade_flow != c("Re-exports", "Re-imports"),] # Remove re-exports and re-imports

# Convert to mirror data
mirror.imports <- subset(comtrade.data, trade_flow == "Imports",
                         c(period, commodity_code, reporter_code, partner_code, netweight_kg, trade_value_usd))
colnames(mirror.imports) <- c("period","commodity_code","partner_code","reporter_code","netweight_kg","trade_value_usd")
...
comtrade.data <- aggregate(trade_value_usd ~ period+commodity_code+reporter_code+partner_code+netweight_kg,
                           comtrade.data, max)
...
rm(x_temp, dx_temp, commodities, reporter, period, mirror.exports, mirror.imports, i, j, k, l)
gc()
...
fwrite(comtrade.data, file = "CleanData/comtrade_monthly_data_201701-202210.csv", sep = ",", col.names = FALSE)
saveRDS(comtrade.data, file = "CleanData/comtrade_monthly_data_201701-202210.rds", compress = TRUE)
