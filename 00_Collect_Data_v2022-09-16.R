...
pacman::p_load("manipulate","gtools","plyr","utils","stats","xlsx","sqldf","tcltk","tcltk2","DescTools","tm","rjson",
               ...
               "lubridate","timeDate","filesstrings","tradestatistics","comtradr","purrr","remotes","rlist")
...
chunk_dates <- function(start_date, end_date, size = 5) {
  force(start_date)
  force(end_date)
  interval(start_date, end_date) -> mainint
  as.period(mainint, unit = "years") -> mainper
  mainper@year -> yearspan
...
# Keep in mind the counter is locally kept. Upstream UN Comtrade API does not tell you how many queries you have left.
ct_wait_for_available <- function(at_least = 2) {
  #ct_register_token(COMTRADE_TOKEN)
  repeat {
    ct_get_remaining_hourly_queries() -> remqueries
    message(paste0( "\n ", at_least, "<=", remqueries, " queries remaining in COMTRADE API query for the hour",
                    "\n Time to next cycle is:", ct_get_reset_time()))
    if (remqueries <= at_least) {
      ct_get_reset_time() - lubridate::now() -> remtime
...
# For the same group of three (reporters, partners, date range), if the input is not "All", then the maximum number of input
# values for each is five. For date range, if not using "All", then the start_date and end_date must not span more than five
# months or five years. There is one exception to this rule, if arg freq is "monthly", then a single year can be passed to
# start_date and end_date and the API will return all of the monthly data for that year. For arg commod_codes, if not using
# input "All", then the maximum number of input values is 20 (although "All" is always a valid input). Additionally, the
# maximum number of returned records from a single query without a token is 50,000. With a token, that number is 250,000.
...
countries <- countries[-1, ]
countries <- as.list(countries[[2]])
countries <- chunk_vector(countries, max.len = 5)
...
comtrade.data<- ddply(x,.(Period, Trade.Flow, Commodity.Code, Commodity, Reporter.Code, Partner.Code),
                      summarise, Value = max(Trade.Value..US.., na.rm = TRUE))

# Expand Reporters and Compute Sums Across Columns
colnames(comtrade.data) <- c('t','Flow','HS6','Description','i','j','v')
ByReporter <- spread(comtrade.data, i, v, fill = 0)
View(ByReporter)

# Save object(s) and remove from memory
exists("comtrade.data")
saveobj("comtrade.data", "comtrade_data_monthly.rdat")

# Remove all unused objects
rm(x_temp, dx_temp, commodities, countries, period)
...
fwrite(append_trade, file = 'RawData/UNComtrade_HS6_Monthly.csv')
