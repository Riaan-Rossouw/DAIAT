  ...
  repeat {
    ...
    if (remqueries <= at_least) {
      ...
      if (as.numeric(wtime) > 0) {
        ...
        Sys.sleep(as.numeric(wtime)) # replaced 'wtime' with 3700
      }
      else {
        break
      }
...
#html_session("http://www.google.com/", use_proxy(proxy.list$proxy_list[5], proxy.list$port[5]))
#content(GET("https://ifconfig.co/json"), "parsed")
#content(GET("https://ifconfig.co/json", use_proxy(proxy.list$proxy_list[2], proxy.list$port[2])), "parsed")
...
## Old For Loop replaced with lapply
for(i in countries){
